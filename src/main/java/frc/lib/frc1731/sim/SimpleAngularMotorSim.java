...
public class SimpleAngularMotorSim {
    ...
    public void setMechanismAngle(Angle angle) {
        ...
        double lastMotorVelocity = converter.toMotor(lastVelocity).in(RadiansPerSecond);
        double motorVelocity = converter.toMotor(getMechanismVelocity()).in(RadiansPerSecond);

        double pidOutput = profiledPID.calculate(motorAngle.in(Rotations), targetMotorAngle.in(Rotations));
        double ffOutput = simFF.calculateWithVelocities(motorAngle.in(Rotations), lastMotorVelocity, motorVelocity);
        appliedVoltage = Volts.of(pidOutput + ffOutput);

        SmartDashboard.putNumber("ASMKLDASMKLDAMKLDAMKL", targetMotorAngle.in(Rotations));
        SmartDashboard.putNumber("9813189021890381290", motorAngle.in(Rotations));
        sim.setVoltage(appliedVoltage);
    }
